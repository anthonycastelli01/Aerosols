...
def get_filename_key_list(directory, filename_pattern):
    """Return a list of files in a directory matching a given pattern
    with matched keys.

    The filename_pattern is a regular expression. All filenames in the
    given directory that match the pattern are returned in a sorted
    list with keys. That is, the returned list consists of tuples
    (filename, key).

    Example:
    >>> keyed_files = partmc.get_filename_list('out/', r'data_(.*)\.nc')
    ...
    for filename in filenames:
        ...
        if match:
            ...
            if len(groups) > 0:
                ...
                output_key = None
            full_filename = os.path.join(directory, filename)
            filename_list.append((full_filename, output_key))
    filename_list.sort()
    if len(filename_list) == 0:
        raise (Exception("No files found in %s matching %s"
                        % (directory, filename_pattern)))
...
def get_time_filename_list(dir, file_pattern):
    ...
    Example:
    >>> for (time, filename, key) in partmc.get_filename_list('out/', r'data_(.*)\.nc'):
             print filename
...
def get_time_layer_filename_list(dir, file_pattern):
    """Return a list of files in a directory matching a given pattern
    with times, layers, and keys.

    The filename_pattern is a regular expression. All filenames in the
    given directory that match the pattern are returned in a structure
    as described below.  The time and layer are determined by opening
    each file as a NetCDF file and reading the information from it. If
    the file_pattern contains a regular expression group then the key
    is the value of that group after the match, otherwise it is None.

    The format of the return value is a list of tuples (time,
    layer_data), where layer_data is a list of tuples of the form
    (altitude, filename, keys, ...). The lists are sorted by
    increasing times or altitudes, as appropriate.

    Example:
    >>> for (time, layer_data) in partmc.get_time_layer_filename_list('out/', r'data_01_01_(.*)_(.*)\.nc'):
            for (altitude, filename, layer_key, time_key) in layer_data:
                print filename
    ...
    for filename in filenames:
        ...
        if match:
            ...
            ncf = scipy.io.netcdf.netcdf_file(netcdf_filename, 'r')
            env_state = env_state_t(ncf)
            if env_state.elapsed_time not in time_dict.keys():
                time_dict[env_state.elapsed_time] = []
            time_dict[env_state.elapsed_time].append(
                [env_state.altitude, netcdf_filename] + list(match.groups()))
            ncf.close()
    time_list = []
    for (time, layer_data) in time_dict.items():
        time_list.append((time, layer_data))
    if len(time_list) == 0:
        raise (Exception("No files found in %s matching %s"
                        % (dir, file_pattern)))
    time_list.sort()
    n_layer = len(time_list[0][1])
    for (time, layer_data) in time_list:
        if len(layer_data) != n_layer:
            raise (Exception("Inconsistent number of layers for times %f and %f"
                            % (time_list[0][0], time)))
        layer_data.sort()
    return time_list
